...
def calcBuyOrder(distributionRate,  # propensity
    ...
    logging.debug(f'calcBuyOrder: b={b}, t={t}, pf={pf}, ai={ai}, vc={vc}, am={am}, ts={ts}')
    ...
    if po < int(min_price) or ao < int(order_min_size):
        logging.debug(f'안사: 주문량:{ao}, 최소주문량:{order_min_size}, 주문가:{po}, 최소주문가:{min_price}')
        po, ao = 0, 0
...
def calcSellOrder(distributionRate,  # propensity
    ...
    am = dcml(order_min_size)  # 최소주문량
    ts = dcml(tick_size)  # 호가단위
    logging.debug(f'calcSellOrder: b={b}, t={t}, pf={pf}, ai={ai}, vc={vc}, am={am}, ts={ts}')
    ...
    if po < int(min_price) or ao < int(order_min_size):
        ...
        logging.debug(f'안팔아: 주문량:{ao}, 최소주문량:{order_min_size}, 주문가:{po}, 최소주문가:{min_price}')
        po, ao = 0, 0
    return {"price": float(po), "amount": float(ao)}
